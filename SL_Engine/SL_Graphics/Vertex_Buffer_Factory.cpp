...
std::shared_ptr<SL_Graphics::Model::IVertex_Buffer> SL_Graphics::Vertex_Buffer_Factory::Create(std::string format){

	auto self = std::make_shared<SL_Graphics::Model::Vertex_Buffer>();
	...
	for (auto& token: splits){
		...
		if (!Model::IVertex_Attribute::Parse(token, attribute.get())) continue;
